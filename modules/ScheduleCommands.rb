...
def initialize
  registerModule("Scheduler", "Schedule commands to run at specified times or intervals.")

  registerCommand("Scheduler", "scheduler add", "Schedule command to run at time. If repeat is given, repeat every 'time' seconds from epoch. If repeat is not given, run in 'time' seconds from now.", "[repeat] time command")
...
def cmd_scheduler(message)
  return unless checkAdmin(message, 2)
