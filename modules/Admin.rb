...
def getSpeakerAccessLevel(message)
  getAccessLevel(message, message.speaker.partialMask)
end

def getAccessLevel(message, hostmask)
  $bot.admins[hostmask]["AccessLevel"] rescue 0
end

def cmd_level(message)
  if message.msgArr.length == 2
    message.reply("Current access level: #{ getAccessLevel(message.msgArr[1])}")
  end
end

def cmd_login(message)
  unless message.private?
    message.reply("You may only use that command in a query.", true)
  else
    if not message.msgArr.length == 3
      message.reply("Please give both a user name and password.")
    else
...
def cmd_schedule(message)
  message.reply($scheduler.schedule.join(", "), true) if checkAdmin(message, 9)
end

def cmd_raw(message)
  sendRaw(message.args) if checkAdmin(message, 9)
end

def cmd_join(message)
  return true unless checkAdmin(message, 5)
...
def cmd_adminadd(message)
  return true unless checkAdmin(message, 9)

  if message.msgArr.length != 4
    message.reply("Usage: adminadd #{UNDERLINE}username#{NORMAL} #{UNDERLINE}password#{NORMAL} #{UNDERLINE}level#{NORMAL}")
    return true
  end  
  ...
  adminUserObj = Hash.new
  adminUserObj["Username"] = message.msgArr[1]
  adminUserObj["Password"] = adminPassword
  adminUserObj["AccessLevel"] = Integer(message.msgArr[3])

  $bot.config["Users"][message.msgArr[1]] = adminUserObj
  message.reply("User #{BOLD}#{message.msgArr[1]}#{NORMAL} added with admin level #{BOLD}#{message.msgArr[3]}#{NORMAL}.")
end
